...
# ---------------------------
async def scrape_url(url: str, prompt: str) -> str:
    """
    Asynchronously scrapes a single webpage and extracts relevant content based on a prompt.

    Args:
        url (str): The URL to scrape.
        prompt (str): Query text used to filter relevant content from the page.

    Returns:
        str: The extracted markdown content from the webpage.
    """
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        if results and results[0].markdown_v2:
            return results[0].markdown_v2.fit_markdown
        else:
